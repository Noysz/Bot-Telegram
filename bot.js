...
// Fungsi web-tools pindah ke modules/web-tools.js. Wire dep yang tetap di bot.js:
// generic-slot semaphore, recordError, + config network/timeout (SCRAPLING_FETCH_URL
// dipakai juga di health-check bot.js). Semua const di bawah udah kedefinisi di atas.
webTools.init({
    acquireGenericSlot,
    releaseGenericSlot,
    recordError,
...
// Wire addfix module (fungsi pipeline pindah ke modules/addfix.js). Inject path
// resolved — ADDFIX_FILE & KB_DIR dipakai juga di bot.js (handler write/rename).
addfix.init({ ADDFIX_FILE, COMMUNITY_KB, KB_DIR });
...
bot.on('message', async (msg) => {
    ...
    if (cmd === '/promotefix') {
        if (!isAdmin(userId)) { sendSafe(chatId, '🔒 Khusus admin.'); return; }
        const pending = addfix.readAddfix();
    ...
    if (videoToProcess) {
        ...
        try {
            ...
            await withTyping(chatId, async () => {
                ...
                const vid = await video.processVideoFile(dlPath);
    ...
    if (ytUrl) {
        try {
            const yt = await withTyping(chatId, () => video.processYouTube(ytUrl));
            if (yt) {
                if (yt.images.length) {
    ...
    if (!ytUrl && !videoToProcess) {
        ...
        if (vUrl && (await video._isAllowedVideoUrl(vUrl))) {
            try {
                const vid = await withTyping(chatId, () => video.processVideoUrl(vUrl));
...
bot.on('callback_query', async (q) => {
    ...
    try {
        ...
        const entry = addfix.takeAddfixByTs(ts);   // sync: baca+hapus dari jsonl (fresh state tiap klik).
        if (!entry) { await ack('Udah diproses.'); await editDone('⚠️ Udah diproses sebelumnya.'); return; }

        if (action === 'p') {
            addfix.promoteAddfix([entry]);          // sanitizeKbBody jalan di sini (gate anti-injeksi ga berubah).
